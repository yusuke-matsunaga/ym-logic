...
)
{
  ...
  if ( PyBdd::Check(other) ) {
    ...
    try {
      return PyBdd::ToPyObject(val & val2);
    }
...
{
  auto& val = PyBdd::_get_ref(self);
  return PyBool_FromLong(val.is_negacube);
}
...
)
{
  auto& val = PyBdd::_get_ref(self);
  try {
    return PyList<Bdd, PyBdd>::ToPyObject(val.to_litlist());
  }
  catch ( std::invalid_argument err ) {
    std::ostringstream buf;
    buf << "invalid argument" << ": " << err.what();
...
PyMethodDef methods[] = {
  ...
  {"is_negacube",
   is_negacube,
   METH_NOARGS,
   PyDoc_STR("return True if negative CUBE")},
  {"check_sup",
...
)
{
  ...
  new (&my_obj->mVal) Bdd(std::move(val));
  return obj;
}
